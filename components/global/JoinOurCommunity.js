export default function JoinOurCommunity({ footer = false }) {
  return (
    <div className="flex md:flex-row flex-col md:space-x-4 md:items-center justify-start space-y-4 md:space-y-0">
      <input
        type="email"
        ...
        } border shadow-xl text-sm rounded-full pt-3.5 px-4 pb-4 md:w-72 h-12 transition-all font-display transition-colors`}
      />
      <button
        type="submit"
        className={`${
